...
function getItemStatus(currentQty, standardQty) {
    ...
    if (std <= 0) return { key: "full", label: "โ๏ธ เน€เธ•เนเธก 100% (Full Stock)", pct: 100, badgeClass: "badge-success" };
    
    const pct = Math.round((current / std) * 100);
    if (pct < 50) return { key: "out_of_stock", label: "๐”ด เธเธฑเธ”เธเธทเนเธญ/เธเธฑเธ”เธซเธฒ (Min. Stock)", pct, badgeClass: "badge-danger" };
    else if (pct >= 50 && pct <= 60) return { key: "low", label: "๐ง เน€เธ•เธทเธญเธ (50-60% Warning)", pct, badgeClass: "badge-orange" };
    else if (pct >= 61 && pct <= 80) return { key: "normal", label: "๐ก เธเธญเธ”เธต (61-80% Fair)", pct, badgeClass: "badge-lime" };
    else if (pct >= 81 && pct <= 99) return { key: "good", label: "๐ข เธ”เธต (81-99% Good)", pct, badgeClass: "badge-lightgreen" };
    else if (pct === 100) return { key: "full", label: "โ๏ธ เน€เธ•เนเธก 100% (Full Stock)", pct, badgeClass: "badge-success" };
    return { key: "over", label: "๐”ต เน€เธเธดเธ 100% (Over Stock)", pct, badgeClass: "badge-blue" };
}
...
class StockDatabase {
    ...
    init() {
        try {
            ...
            console.error("Error initializing items:", e);
        }
    }
    ...
    async pushToCloudflare() {
        try {
            ...
            console.log("Cloudflare sync notice (push):", err);
        }
    }
    ...
    async syncFromCloudflare() {
        try {
            ...
            // 2. Sync History Transaction Logs from Cloudflare D1
            ...
            if (logRes.ok) {
                ...
                if (dbLogs && Array.isArray(dbLogs)) {
                    const formattedDbLogs = dbLogs.map(l => ({
                        ...
                        type: l.type || 'dispense',
                        itemCode: l.itemCode || l.item_code || '',
                        itemName: l.itemName || l.item_name || '',
                        qty: Number(l.qty || 0),
                        unit: l.unit || 'เธเธดเนเธ',
                        balanceBefore: l.balanceBefore !== undefined ? Number(l.balanceBefore) : undefined,
                        currentStock: Number(l.currentStock || l.current_stock || 0),
                        requester: l.requester || '',
                        workOrder: l.workOrder || l.work_order || '',
                        note: l.note || ''
                    }));

                    formattedDbLogs.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
                    const currentLocalLogs = this.getLogs();
                    if (JSON.stringify(currentLocalLogs) !== JSON.stringify(formattedDbLogs)) {
                        this.saveLogs(formattedDbLogs);
                        hasChanges = true;
            ...
            console.log("Cloudflare sync notice (pull):", e);
            return false;
        }
    }
    ...
    getItems() {
        try {
            const data = localStorage.getItem(this.STORAGE_KEY_ITEMS);
            if (!data || !Array.isArray(JSON.parse(data)) || JSON.parse(data).length === 0) {
                this.init();
                const fresh = localStorage.getItem(this.STORAGE_KEY_ITEMS);
                return fresh ? JSON.parse(fresh) : MASTER_ITEMS;
            }
    ...
    async processTransaction(type, code, qty, requester = "-", workOrder = "-", note = "") {
        if (type === "audit" && !this.hasAuditPermission()) throw new Error("Admin permission required");

        const items = this.getItems();
        const itemIndex = items.findIndex(i => i.code === code);
        if (itemIndex === -1) throw new Error("Item code not found " + code);
        ...
        if (type === "dispense") {
            newQty = Math.max(0, oldQty - changeQty);
        ...
        return {
            ...
            unit: updatedItem.unit || "เธเธดเนเธ",
    ...
    updateStock(code, type, qty, requester = "", workOrder = "", note = "") {
        return this.processTransaction(type, code, qty, requester, workOrder, note);
    }

    async updateSingleItemCloudflare(code, currentQty, mb52Qty, wmsQty, kk23Qty, imageUrl, log) {
        try {
            const baseUrl = this.getApiBaseUrl();
            await fetch(${baseUrl}/api/update, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ code, currentQty, mb52Qty, wmsQty, kk23Qty, imageUrl, log })
            });
        } catch (e) {
            console.log("Cloudflare single item sync notice:", e);
        }
    }

    updateLocationQuantities(code, mb52Qty, wmsQty, kk23Qty = 0) {
        const items = this.getItems();
        ...
        this.saveItems(items);
        this.updateSingleItemCloudflare(code, item.currentQty, item.mb52Qty, item.wmsQty, item.kk23Qty, item.imageUrl);
        return item;
    }

    importLocationQuantitiesBatch(batchDataList) {
        if (!Array.isArray(batchDataList) || batchDataList.length === 0) return 0;

        const items = this.getItems();
        let updatedCount = 0;
        let updateSummaryList = [];

        batchDataList.forEach(data => {
            const item = items.find(i => i.code === data.code);
            if (item) {
                const oldMb52 = item.mb52Qty || 0;
                const oldWms = item.wmsQty || 0;
                const oldKk23 = item.kk23Qty || 0;
                const newMb52 = Number(data.mb52Qty || 0);
                const newWms = Number(data.wmsQty || 0);
                const newKk23 = Number(data.kk23Qty || 0);

                if (oldMb52 !== newMb52 || oldWms !== newWms || oldKk23 !== newKk23) {
                    item.mb52Qty = newMb52;
                    item.wmsQty = newWms;
                    item.kk23Qty = newKk23;
                    item.lastUpdated = new Date().toISOString();
                    updatedCount++;
                    updateSummaryList.push(${item.code}: MB52=, WMS=);
                }
            }
        });
        ...
        if (updatedCount > 0) {
            this.saveItems(items);
            this.pushToCloudflare();
            const summaryNote = เธเธณเน€เธเนเธฒ Excel เธขเธญเธ” MB52/WMS เธชเธณเน€เธฃเนเธ  เธฃเธฒเธขเธเธฒเธฃ;
            const batchLog = {
                id: Date.now(),
                timestamp: new Date().toISOString(),
                type: "audit",
                itemCode: "BATCH_IMPORT",
                itemName: summaryNote,
                qty: updatedCount,
                unit: "เธฃเธฒเธขเธเธฒเธฃ",
                balanceBefore: 0,
                currentStock: updatedCount,
                requester: "เน€เธเนเธฒเธเธญเธเธฃเธฐเธเธ (Excel Import)",
                workOrder: "IMPORT_EXCEL",
                note: updateSummaryList.slice(0, 10).join("; ") + (updateSummaryList.length > 10 ? "..." : "")
            };
            const logs = this.getLogs();
            logs.unshift(batchLog);
            this.saveLogs(logs);
        }
    ...
    getRequesters() {
        try {
            const data = localStorage.getItem(this.STORAGE_KEY_REQUESTERS);
            return data ? JSON.parse(data) : DEFAULT_REQUESTERS_LIST;
        } catch (e) {
    ...
    getAuditPermission() {
        return localStorage.getItem(this.STORAGE_KEY_AUDIT_PERM) === "true";
    }

    hasAuditPermission() {
        return this.getAuditPermission();
    }

    setAuditPermission(allowed) {
        localStorage.setItem(this.STORAGE_KEY_AUDIT_PERM, allowed ? "true" : "false");
    }

    verifyOwnerPin(pin) {
        const savedPin = localStorage.getItem(this.STORAGE_KEY_OWNER_PIN) || "1234";
        return pin === savedPin;
    }

    setOwnerPin(newPin) {
        localStorage.setItem(this.STORAGE_KEY_OWNER_PIN, newPin);
    }
    ...
    resetToDefault() {
        ...
        localStorage.removeItem("pea_warehouse_zeroed_v1");
